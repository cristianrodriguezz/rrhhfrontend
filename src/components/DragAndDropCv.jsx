...
const DragAndDropCv = ({ onFileUpload, formik }) => {
  ...
  const onDrop = useCallback((acceptedFiles) => {
    if (acceptedFiles.length === 1) {
      ...
      if (file.name.toLowerCase().endsWith(".pdf") || file.name.toLowerCase().endsWith(".docx") ||  file.name.toLowerCase().endsWith(".jpg") ) {
        setError("")
        ...
        <div className="flex flex-col items-center justify-center pt-5 pb-6 w-full *:text-center">
          {
            ...
            <>
              ...
              <p className="text-xs">solo se admite archivo (PDF, DOCX, JPG)</p>
            </>
            :
